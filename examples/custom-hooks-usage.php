...
add_filter( 'clicksync_should_sync_customer', 'my_custom_clicksync_customer_eligibility', 10, 2 );
function my_custom_clicksync_customer_eligibility( $should_sync, $customer_id ) {
...
add_filter( 'clicksync_order_payload', 'my_custom_clicksync_order_payload_decorator', 10, 2 );
function my_custom_clicksync_order_payload_decorator( $payload, $order ) {
    // Inject ACF metadata
...
add_filter( 'clicksync_api_request_headers', 'my_custom_clicksync_headers', 10, 2 );
function my_custom_clicksync_headers( $headers, $endpoint ) {
    $headers['X-Developer-Token'] = 'CS-DEV-9994827';
    return $headers;
}

// 5. Post-Dispatch Hook: Perform custom logic when an order successfully syncs
add_action( 'clicksync_event_dispatched', 'my_custom_clicksync_post_dispatch', 10, 3 );
function my_custom_clicksync_post_dispatch( $topic, $payload, $response ) {
