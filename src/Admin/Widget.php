...
class Widget {
	...
	/**
	 * Render WooCommerce Edit Order ShopMotive Meta Box.
	 *
	...
	public static function render_order_metabox( $post ) {
		...
		$last_sync_formatted = $last_sync ? date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $last_sync ) : __( 'Never', 'shopmotive-crm-sync-for-clickup' );
		?>
			...
			<?php else : ?>
				<div style="margin-bottom: 12px;">
					...
					<span style="display: block; font-size: 11px; color: #64748b; margin-top: 4px;">
						...
						printf( esc_html__( 'Last Sync: %s', 'shopmotive-crm-sync-for-clickup' ), esc_html( $last_sync_formatted ) );
						?>
					</span>
				</div>
				...
				<div style="margin-bottom: 10px;">
					<label style="font-weight: 600; display: block; margin-bottom: 4px; font-size: 11px; color: #475569;"><?php esc_html_e( 'ClickUp Status:', 'shopmotive-crm-sync-for-clickup' ); ?></label>
					<select class="clicksync-widget-status-select clicksync-select" style="width: 100%;">
						<option value=""><?php esc_html_e( 'Loading task statuses...', 'shopmotive-crm-sync-for-clickup' ); ?></option>
					</select>
				</div>

				<!-- Priority Select -->
				<div style="margin-bottom: 10px;">
					<label style="font-weight: 600; display: block; margin-bottom: 4px; font-size: 11px; color: #475569;"><?php esc_html_e( 'ClickUp Priority:', 'shopmotive-crm-sync-for-clickup' ); ?></label>
					<select class="clicksync-widget-priority-select clicksync-select" style="width: 100%;">
						<option value=""><?php esc_html_e( 'Loading priorities...', 'shopmotive-crm-sync-for-clickup' ); ?></option>
					</select>
				</div>

				<!-- Assignee List Checkboxes -->
				<div style="margin-bottom: 14px;">
					<label style="font-weight: 600; display: block; margin-bottom: 4px; font-size: 11px; color: #475569;"><?php esc_html_e( 'ClickUp Assignees:', 'shopmotive-crm-sync-for-clickup' ); ?></label>
					<div class="clicksync-widget-assignees-list" style="background: #fafafa; border: 1px solid #e2e8f0; border-radius: 6px; padding: 8px; max-height: 120px; overflow-y: auto;">
						<span style="font-size: 11px; color: #64748b;"><?php esc_html_e( 'Loading members...', 'shopmotive-crm-sync-for-clickup' ); ?></span>
					</div>
				</div>
				...
				<button type="button" class="clicksync-manual-sync-btn button button-secondary" style="width: 100%; display: flex; align-items: center; justify-content: center; gap: 6px; height: 36px; line-height: 34px; font-weight: 600;">
					...
					<span><?php esc_html_e( 'Force Sync Now', 'shopmotive-crm-sync-for-clickup' ); ?></span>
				</button>
			<?php endif; ?>
		</div>
		<?php
	...
	/**
	 * Render WooCommerce Edit User (Customer) ShopMotive Meta Box.
	 *
		...
		<div class="clicksync-widget-wrapper" data-customer-id="<?php echo esc_attr( $customer_id ); ?>" data-task-id="<?php echo esc_attr( $task_id ); ?>" style="font-size: 13px; color: #475569; line-height: 1.5; max-width: 320px;">
			...
			<div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 12px; margin-bottom: 12px; font-size: 12px;">
				<div style="margin-bottom: 8px; border-bottom: 1px solid #e2e8f0; padding-bottom: 6px;">
					<span style="font-weight: 600; font-size: 11px; text-transform: uppercase; color: #64748b;"><?php esc_html_e( 'Customer Summary', 'shopmotive-crm-sync-for-clickup' ); ?></span>
				</div>
				<div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
					<span style="color: #64748b;"><?php esc_html_e( 'Total Spend:', 'shopmotive-crm-sync-for-clickup' ); ?></span>
				...
				<div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
					<span style="color: #64748b;"><?php esc_html_e( 'Total Orders:', 'shopmotive-crm-sync-for-clickup' ); ?></span>
				...
				<?php if ( ! empty( $recent_orders ) ) : ?>
					<div style="margin-top: 10px; border-top: 1px dashed #cbd5e1; padding-top: 8px;">
						<span style="font-weight: 600; font-size: 10px; text-transform: uppercase; color: #64748b; display: block; margin-bottom: 4px;"><?php esc_html_e( 'Recent Orders', 'shopmotive-crm-sync-for-clickup' ); ?></span>
