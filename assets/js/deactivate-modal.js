jQuery(document).ready(function($) {
    ...
    var modalHtml = `
    <div id="swiftsync-deactivate-modal" style="display:none; position:fixed; z-index:999999; left:0; top:0; width:100%; height:100%; background:rgba(15, 23, 42, 0.7); backdrop-filter:blur(4px); align-items:center; justify-content:center;">
        <div style="background:#ffffff; width:100%; max-width:520px; border-radius:12px; box-shadow:0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04); font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; overflow:hidden; border:1px solid #e2e8f0; animation: swiftsync-fade-in 0.2s ease-out; box-sizing:border-box;">
            ...
            <div style="padding:20px 24px; border-bottom:1px solid #e2e8f0; display:flex; justify-content:space-between; align-items:center; background:#f8fafc;">
                <h3 style="margin:0; font-size:16px; font-weight:700; color:#0f172a; display:flex; align-items:center; gap:10px; line-height:1.2;">
                    ${logoUrl ? `<img src="${logoUrl}" alt="SwiftSync" style="width:24px; height:24px; border-radius:6px; display:inline-block; flex-shrink:0; vertical-align:middle;" />` : ''}
                    <span style="display:inline-block; vertical-align:middle;">Deactivate SwiftSync?</span>
                </h3>
                <button type="button" id="swiftsync-modal-close" style="background:none; border:none; font-size:20px; color:#64748b; cursor:pointer; line-height:1;">&times;</button>
            </div>
            ...
            <div style="padding:24px; box-sizing:border-box;">
                ...
                <div style="margin-bottom:20px; width:100%; box-sizing:border-box;">
                    ...
                    <textarea id="swiftsync-exit-message" rows="3" placeholder="Tell us how we can improve SwiftSync..." style="width:100% !important; max-width:100% !important; min-width:100% !important; box-sizing:border-box !important; margin:0 !important; padding:10px 14px; border:1px solid #cbd5e1; border-radius:6px; font-size:13px; color:#1e293b; resize:vertical; outline:none; font-family:inherit; display:block !important;"></textarea>
                </div>
                ...
                <div style="background:#f1f5f9; padding:12px 16px; border-radius:8px; margin-bottom:20px; display:flex; align-items:center; gap:10px; border:1px solid #e2e8f0; box-sizing:border-box;">
                    <input type="checkbox" id="swiftsync-cancel-sub-check" style="margin:0; width:16px; height:16px; accent-color:#008060; cursor:pointer;" />
                    <label for="swiftsync-cancel-sub-check" style="font-size:13px; font-weight:600; color:#0f172a; cursor:pointer; margin:0;">
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; gap:10px; flex-wrap:wrap; border-top:1px solid #e2e8f0; padding-top:16px; box-sizing:border-box;">
                    <a href="${deactivateUrl}" id="swiftsync-skip-deactivate" style="font-size:12px; color:#64748b; text-decoration:underline; cursor:pointer;">
                        Skip & Deactivate
                    </a>

                    <div style="display:flex; gap:8px;">
                        <button type="button" id="swiftsync-submit-deactivate" style="padding:8px 16px; font-size:13px; font-weight:600; background:#008060; color:#ffffff; border:none; border-radius:6px; cursor:pointer;">
                            Submit & Deactivate
                        </button>
                    </div>
                </div>
    ...
    <style>
        @keyframes swiftsync-fade-in {
            from { opacity: 0; transform: scale(0.96); }
            to { opacity: 1; transform: scale(1); }
        }
        #swiftsync-exit-reason:focus, #swiftsync-exit-message:focus {
    ...
    $deactivateLink.on('click', function(e) {
        e.preventDefault();
        $('#swiftsync-deactivate-modal').css('display', 'flex');
    });

    // Modal Close buttons
    $('#swiftsync-modal-close').on('click', function() {
        $('#swiftsync-deactivate-modal').hide();
    });

    // Submit & Deactivate handler
    $('#swiftsync-submit-deactivate').on('click', function(e) {
        e.preventDefault();
        var $btn = $(this);
        $btn.text('Processing...').prop('disabled', true);

        var reason = $('#swiftsync-exit-reason').val();
        var message = $('#swiftsync-exit-message').val();
        var canceledSub = $('#swiftsync-cancel-sub-check').is(':checked');

        var data = {
            action: 'swiftsync_submit_exit_feedback',
